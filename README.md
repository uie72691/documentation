...
Example for Linux: cip_bti_conan/verify.sh at master · ADAS-3RDPARTY/cip_bti_conan (conti.de)
Example for Windows: cip_bti_conan/verify.ps1 at master · ADAS-3RDPARTY/cip_bti_conan (conti.de)
  
10. Set the environment variables ARTIFACTORY_USER (value your username) and ARTIFACTORY_API_KEY (value is your Artifactory API-Key https://eu.artifactory.conti.de/artifactory/webapp/#/profile), these are needed to create the package locally.
...
13. Push the change to the GitHub (http://github.conti.de/ADAS-3RDPARTY/cip_bti_conan) repository. The Jenkins server will create the Conan package and upload it to the Artifactory Repository: c_adas_cip_conan_stage_l if you merge your PR then jenkins will upload your package to the productive conan repository: c_adas_cip_conan_l.
...
Search the available packages with: conan search <package name> -r <remote>.
  Example:
  ```PowerShell
...
To install a Conan package run: "conan install <package name> -r <remote>".
  Example: 
  ```PowerShell
...
> **Warning**: Note this will only add your tool to the build process.  You will actually call it in your cmake build.
